pro hectosky_n2fix,pointname,quick=quick
...
;           ***This version fixes the wavelength of [NII] 6547.
;
...
for nn=1,nlines-1 do begin
   ...
   ; ## FIX-IT FOR [NII] 6547
   ; #################################################################
   if (neblines[nn].lambda eq 6547) then begin
      ; Fix the center of the line for this one only.
      parinfo[1].fixed = 1
      parinfo[1].value = 6545.6
      ; ### APPROX. VALUE FROM 2011.1025 MASTER SKY ###
      neblines[nn].estimate[1] = parinfo[1].value
   endif else begin
      parinfo[1].fixed = 0
   endelse
   ; #################################################################
