pro hectosky_one,pointname,myap,myscale,quick=quick
...
;           ***This version operates on a SINGLE OBJECT ONLY and can
;              scale the sky lines.
;
; INPUT:    pointname    ID name of Hectospec pointing/configuration,
;                        e.g., 'W3_14.0_16.5_v3_1'
;           myap         Aperture of the object you want to fiddle
;                        around with. 
;           myscale      Scaling factor for NEBULAR LINES ONLY. 
...
; ### CHANGED BECAUSE WE DON'T WANT TO DELETE THE EXISTING SET
; ### OF FILES.
newobjdir = pointname + '.' + string(myap,format='(I3.3)') + '.skysub/'
   ...
   if (dirlist[0] ne '') then begin
      ...
      if (re1 eq 'y') then begin
         ...
         read,'Enter new folder name for sky-subtracted results: ',newobjdir
                                ; REQUIRES THAT YOU TYPE THE / AT THE
...
; ### ACTUALLY: go over only the one object you care about
; and want to mess with in this round. ###
iwant = where(star_ap eq myap)
; Make sure you put in a valid aperture number.
if (iwant[0] eq -1) then begin
   print,'INVALID APERTURE NUMBER.'
   return
endif

for jj=iwant[0],iwant[0] do begin
   ...
   ; ### Scale the H-alpha line by the desired factor.
   neblines[0].fit[3] = temporary(neblines[0].fit[3])*myscale
   ; Then compute the shape of the line.
   neblines[0].line = mygauss(wavel,neblines[0].fit)
   ...
   for nn=1,nlines-1 do begin
      ...
      ; ### Scale the lines byt the desired factor. 
      neblines[nn].fit[3] = temporary(neblines[nn].fit[3])*myscale
      ; Then compute the shape of the line.  
      neblines[nn].line = mygauss(wavel,neblines[nn].fit)
   endfor
   ...
   xr_bot = [6500,6650]
   inxr = where((wavel ge 6500) and (wavel le 6650))
   ...
   if (not keyword_set(quick)) then begin
      ...
      p1 = ''
      read,'Hit ENTER to exit the program. ',p1
